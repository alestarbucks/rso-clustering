# original module by tugrulhkarabulut
# modified by Alejandro Álvarez Conejo
# https://github.com/tugrulhkarabulut/K-Means-Clustering

from re import I
import numpy as np

import k_means.initialization_methods as init_methods

class KMeans:

    def __init__(self, n_clusters = 3, tolerance = 0.01, max_iter = 100, runs = 1, init_method="forgy", distance=None):
        ...
        if distance is None:
            self.distance = lambda a, b : np.linalg.norm(a - b, axis=1)
        else:
    ...
    def __compute_distances(self, X, cluster_means, row_count):
        ...
        for cluster_mean_index, cluster_mean in enumerate(cluster_means):
            distances[:, cluster_mean_index] = self.distance(X, cluster_mean)
